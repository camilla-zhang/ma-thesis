---
title: "Robustness check: Disapproval ratings"
...
```
### Test subset data (to ensure data transforming/merging was successful)
```{r}
...
gallup = subset(gallup, Disapprove != "NA")

#Remove percentages in approval rating variable
gallup$Disapprove <- as.integer(gsub('%', '', gallup$Disapprove))
...
depth_data2 = depth_data2[!is.na(depth_data2$Disapprove),]
...
```{r}
fe_mod1 <- plm(Disapprove ~ Lag(sqrt(depth_filled), shift = 2) +
                  developing +
                  Lag(log(gdp)) +
                  Lag(edu) +
                  Lag(v2x_regime) +
                  year + 
                  Lag((depth_filled), shift = 1)*developing,
...
count_data2 = count_data2[!is.na(count_data2$Disapprove),]
```

### Model 1: Change in PTA membership count
```{r}
#Build FE model on PTA count
fe_mod2 <- plm(Disapprove ~ Lag(total_ptas, shift = 2) + 
                 Lag(log(gdp)) + 
                 Lag(edu) + 
                 Lag(v2x_regime) + 
                 year + 
...
#Remove NAs 
deep_count_data = deep_count_data[!is.na(deep_count_data$Disapprove),]

#Subset to 2005-2021 and rows without missing data
deep_count_data2 = deep_count_data[deep_count_data$year > 2004,]
deep_count_data2 = deep_count_data2[!is.na(deep_count_data2$Disapprove),]
```

```{r}
#Build FE model on PTA count
fe_mod3 <- plm(Disapprove ~ 
