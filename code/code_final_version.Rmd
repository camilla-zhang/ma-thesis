...
### Model 1: Depth w/Lags and logged Depth/GDP
```{r}
fe_mod1 <- plm(Approve ~ Lag(sqrt(depth_filled), shift = 2) +
                  developing +
                  Lag(log(gdp)) +
                  Lag(edu) +
                  Lag(v2x_regime) +
                  year + 
                  Lag(sqrt(depth_filled), shift = 1)*developing,
...
fe_mod2 <- plm(Approve ~ Lag(total_ptas, shift = 2) + 
                 Lag(log(gdp)) + 
                 Lag(edu) + 
                 Lag(v2x_regime) + 
                 year + 
                 Lag(total_ptas, shift = 2)*developing,
                    data = count_data2,
                    index = c("iso3n", "year"), 
                    model = "within",
                    effect = "twoways")
...
```{r}
#Test if we need to control for individual effects
plmtest(fe_mod1, effect="individual", type="bp")
plmtest(fe_mod2, effect="individual", type="bp")
plmtest(fe_mod3, effect="individual", type="bp")

#Test if we need to control for time effects
plmtest(fe_mod1, effect="time", type="bp")
plmtest(fe_mod2, effect="time", type="bp")
plmtest(fe_mod3, effect="time", type="bp")
```

# Summary Statistics Pt 2

## Average PTA memberships per country over time b/w developed and developing
```{r}
#Count number of PTAs for each country type-year (developing/not developing)

country_yr_n = count_data %>% group_by(developing3,year)  %>% summarise(n = mean(total_ptas))
country_yr_n = subset(country_yr_n, !is.na(developing3)) #Remove NA rows
country_yr_n = subset(country_yr_n, !is.na(n))

# Average PTAs per year between 1990-2021 by developing country
count_dy = ggplot(country_yr_n, aes(x = year, y = n, color = as.factor(developing3), group = as.factor(developing3))) +
  geom_line()+
  geom_point()+
  scale_color_manual(values=c('Black','Dark Grey'))+
...
```{r}
count_data_ds = subset(count_data, year < 2022 & year > 2004) %>% select(iso3n, Approve, total_ptas, gdp, edu, v2x_regime, developing)
summary_table = psych::describe(count_data_ds)


count_data_developing = subset(count_data_ds, developing == 1)
count_data_developed = subset(count_data_ds, developing == 0)

summary_table_developing = psych::describe(count_data_developing)
summary_table_developed = psych::describe(count_data_developed)

summary_table
summary_table_developing
summary_table_developed

#Obtain average number of deep trade agreements and by country-type
deep_count_data_ds = subset(deep_count_data, year < 2022 & year > 2004) %>% select(iso3n, total_ptas, developing) %>% rename(deep_ptas = total_ptas)  
deep_summary_table = psych::describe(deep_count_data_ds)
deep_count_data_developing = subset(deep_count_data_ds, developing == 1)
deep_count_data_developed = subset(deep_count_data_ds, developing == 0)
deep_summary_table_developing = psych::describe(deep_count_data_developing)
deep_summary_table_developed = psych::describe(deep_count_data_developed)
deep_summary_table
deep_summary_table_developing
deep_summary_table_developed
```


# Robustness Check 1: RE and Hausman Test
```{r}
re_mod1 <- plm(Approve ~ Lag(sqrt(depth_filled), shift = 2) +
                  developing +
                  Lag(log(gdp)) +
                  Lag(edu) +
                  Lag(v2x_regime) +
                  year +
                  Lag(sqrt(depth_filled), shift = 2)*developing,
                    data = depth_data2,
                    index = c("iso3n", "year"), 
                    model = "random")
summary(re_mod1)
...
#Build RE model on PTA count
re_mod2 <- plm(Approve ~ Lag(total_ptas, shift = 2) + 
                 Lag(log(gdp)) + 
                 Lag(edu) + 
                 Lag(v2x_regime) + 
                 year + 
...
#Build RE model on deep PTA count
re_mod3 <- plm(Approve ~ 
                 Lag((total_ptas), shift = 2) + 
                 Lag(log(gdp)) + 
                 Lag(edu) + 
...
# Robustness Check 2: Deep Trade Agreement = index of 4+
```{r}
#obtain a list of PTAs that have a depth > mean of depth
deep_ptas = subset(desta, desta$depth_index > 4)$base_treaty
...
#Subset to 2005-2021 and rows without missing data
deep_count_data3 = deep_count_data[deep_count_data$year > 2004,]
deep_count_data3 = deep_count_data3[!is.na(deep_count_data3$Approve),]
